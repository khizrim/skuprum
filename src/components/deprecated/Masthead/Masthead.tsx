...
export const Masthead = component$<MastheadProps>(() => {
  ...
  return (
    <section class="relative px-4 py-8 sm:px-6 lg:px-12 max-w-6xl mx-auto text-left">
      <h1 class="sr-only">Сдать металл на переработку — цена за кг</h1>

      <div class="relative z-10 flex flex-col-reverse lg:flex-row items-center lg:items-start gap-8">
        ...
        <div class="absolute top-0 right-0 transform -translate-y-20 translate-x-1/3 w-full aspect-square lg:w-2/3 sm:block hidden">
          <MetalImage
            src={
              metalOptions.find((m) => m.label === selectedMetal.value)
                ?.image || ''
            }
